...
크롤러는 페이지 요청 중 네트워크 오류, HTTP 오류, 알라딘 오류 HTML, 첫 페이지의 상품 링크 누락을 일시 오류로 판단한다. 최초 요청이 실패하면 최대 3회까지 재시도하며, 최초 요청을 포함하면 한 페이지당 최대 4번 시도한다. 재시도 간격은 3초·6초·12초이고, 모두 실패하면 `FAILED`로 종료해 잘못된 HTML을 Gold나 MySQL에 반영하지 않는다.
...
일시 오류가 발생하면 위 페이지 출력 대신 `재시도 1/3`, `재시도 2/3`, `재시도 3/3` 메시지가 로그에 남는다. 정상 응답으로 회복되면 이후 페이지 수집을 계속하고, 네 번의 시도 모두 실패하면 `FAILED`가 기록된다.
...
2026-09-08에는 오전 9시 자동 실행이 알라딘 오류 페이지를 받아 실패했지만, 재시도 로직을 추가한 뒤 수동 복구 실행을 완료했다. 20페이지에서 1,000개 상품을 수집했고, Gold 도서 7,807행·시리즈 3,600행을 생성했다. MySQL에는 `books` 1,599개·`book_snapshots` 7,807행·`series_daily_stats` 3,600행이 반영됐으며, 두 통계 테이블의 날짜별 중복 그룹은 0개였다.
...
현재 등록된 작업은 매일 오전 9:00으로 설정되어 있다. 오전 8:00 실행을 원하면 작업 스케줄러의 트리거 시간을 별도로 변경해야 한다. 일시 오류 재시도는 작업 스케줄러가 새 작업을 만드는 방식이 아니라, 실행 중인 크롤러가 해당 페이지를 최대 3회 다시 요청하는 방식이다.
...
일시 오류라면 `페이지 일시 오류 (재시도 1/3)` 같은 메시지가 먼저 기록된다. 3회의 재시도까지 모두 실패하면 해당 실행은 `FAILED`로 끝나며 Gold와 MySQL은 실행되지 않는다. `CRAWL START` 뒤에 재시도 메시지 없이 바로 실패한다면 로그의 마지막 오류와 오늘자 raw HTML을 함께 확인한다.
